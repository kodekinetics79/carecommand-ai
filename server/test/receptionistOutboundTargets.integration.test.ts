...
/**
 * The reminder journey, end to end, against the shape the four Phase-1 branches
 * produce TOGETHER.
 *
 * This is the assertion none of the individual branches could make. The
 * appointment context was written against the dial code while it lived in
 * outbound.ts; the dialler moved that code wholesale into outboundLaunch.ts.
 * Both PRs were green in isolation and the context could have been dropped
 * entirely by the merge without a single test noticing — a reminder call that
 * silently stopped mentioning the appointment, which is the whole product.
 *
 * So this asserts the PROVIDER REQUEST, the only place the truth is visible.
 */
describe('a reminder call speaks the patient\'s own appointment', () => {
  async function dialAndCaptureVariables(tenant: TenantFixture, campaignId: string, targetId: string, phone: string) {
    const box: { vars: Record<string, unknown> | null } = { vars: null };
    const providerFetch = vi.fn<typeof fetch>(async (url, init) => {
      if (String(url).includes('/v2/create-phone-call')) {
        const body = JSON.parse(String((init as RequestInit).body)) as Record<string, unknown>;
        box.vars = body.retell_llm_dynamic_variables as Record<string, unknown>;
        return new Response(JSON.stringify({
          call_id: randomRetellCallId('retell_reminder'), agent_id: tenant.providerAgentId, agent_version: 1,
        }), { status: 201, headers: { 'content-type': 'application/json' } });
      }
      return new Response(null, { status: 404 });
    });
    ...
    const response = await app.inject({
      method: 'POST', url: `/v1/receptionist/outbound-campaigns/${campaignId}/call`, headers: auth(tenant),
      payload: { targetId, phone },
    });
    // 201: the call was accepted and a call log created.
    expect(response.statusCode, JSON.stringify(response.json())).toBe(201);
    return box.vars;
  }

  it('sends that appointment, and an unbound target sends empty strings rather than a placeholder', async () => {
    const tenant = await makeTenant();
    const campaignId = await createCampaign(tenant, { status: 'RUNNING' });

    // Bound target: a real appointment on this patient.
    const bound = await addPatientTarget(tenant, campaignId, 9101);
    const startsAt = new Date(Date.now() + 3 * 86_400_000);
    const appointment = await db.appointment.create({
      data: {
        tenantId: tenant.id, branchId: tenant.branchId, patientId: bound.patientId!,
        service: 'New patient cleaning', startsAt, endsAt: new Date(startsAt.getTime() + 30 * 60_000),
        status: 'CONFIRMED', channel: 'CALL',
      },
    });
    await db.receptionistCallTarget.update({ where: { id: bound.id }, data: { appointmentId: appointment.id } });

    const boundVars = await dialAndCaptureVariables(tenant, campaignId, bound.id, bound.phone);
    expect(boundVars, 'the provider request carried no dynamic variables at all').toBeTruthy();
    // The identity of the appointment, so confirm_appointment has something to act on.
    expect(boundVars!.appointment_id).toBe(appointment.id);
    // And something the agent can actually say.
    expect(String(boundVars!.appointment_service)).toContain('New patient cleaning');
    expect(String(boundVars!.appointment_date)).not.toBe('');

    // Unbound target: every key still present, every value empty. An absent key
    // or a placeholder is what lets an agent improvise an appointment.
    const unbound = await addPatientTarget(tenant, campaignId, 9102);
    const unboundVars = await dialAndCaptureVariables(tenant, campaignId, unbound.id, unbound.phone);
    expect(unboundVars).toBeTruthy();
    for (const key of ['appointment_id', 'appointment_service', 'appointment_date', 'appointment_time']) {
      expect(Object.keys(unboundVars!), `${key} must be present even when unbound`).toContain(key);
      expect(unboundVars![key], `${key} must be empty, never a placeholder`).toBe('');
    }
  }, 30_000);

  it('says nothing rather than something stale when the appointment was cancelled after binding', async () => {
    const tenant = await makeTenant();
    const campaignId = await createCampaign(tenant, { status: 'RUNNING' });
    const target = await addPatientTarget(tenant, campaignId, 9103);
    const startsAt = new Date(Date.now() + 3 * 86_400_000);
    const appointment = await db.appointment.create({
      data: {
        tenantId: tenant.id, branchId: tenant.branchId, patientId: target.patientId!,
        service: 'Check-up', startsAt, endsAt: new Date(startsAt.getTime() + 30 * 60_000),
        status: 'CONFIRMED', channel: 'CALL',
      },
    });
    await db.receptionistCallTarget.update({ where: { id: target.id }, data: { appointmentId: appointment.id } });
    // Cancelled between building the list and dialling it.
    await db.appointment.update({ where: { id: appointment.id }, data: { status: 'CANCELED' } });

    const vars = await dialAndCaptureVariables(tenant, campaignId, target.id, target.phone);
    expect(vars).toBeTruthy();
    expect(vars!.appointment_id, 'a cancelled appointment must not be spoken').toBe('');
    expect(vars!.appointment_service).toBe('');
  }, 30_000);
});
