...
import { clinicFixtureData } from './helpers/receptionistFixtures';
...
async function tenantWithAgent(input: { expiresInMs: number; status?: 'VERIFIED' | 'INVALID'; active?: boolean }) {
  ...
  const clinic = await db.receptionistClinic.create({
    data: clinicFixtureData({ tenantId, name: `Clinic ${tenantId.slice(0, 8)}`, phone: `+1212${tenantId.replace(/\D/g, '').slice(0, 7)}`, active: true }) as never,
  });
