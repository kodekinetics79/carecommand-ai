...
describe('AI receptionist — real-time tools', () => {
  it('checks live availability then books, and texts back a confirmation message', async () => {
    ...
    const avail = await handleAgentTool(ctx, 'check_availability', { appointment_date: date }) as { available: boolean; slots: { time: string }[]; message: string };
    expect(avail.available).toBe(true);
    expect(avail.slots.length).toBeGreaterThan(0);
    expect(avail.message).toMatch(/which works/i);

    const slot = avail.slots[0].time;
    const book = await handleAgentTool(ctx, 'book_appointment', { first_name: 'Jane', last_name: 'Doe', phone: '+15551230000', appointment_date: date, appointment_time: slot, service: 'Cleaning' }) as { booked: boolean; appointment_id: string; message: string };
    ...
    const avail2 = await handleAgentTool(ctx, 'check_availability', { appointment_date: date }) as { slots: { time: string }[] };
  ...
  it('is idempotent — the same call cannot double-book the same slot', async () => {
    ...
    const avail = await handleAgentTool(ctx, 'check_availability', { appointment_date: date }) as { slots: { time: string }[] };
    const slot = avail.slots[0].time;
    const args = { first_name: 'Sam', last_name: 'Lee', appointment_date: date, appointment_time: slot };
    await handleAgentTool(ctx, 'book_appointment', args);
    const dup = await handleAgentTool(ctx, 'book_appointment', args) as { booked: boolean; duplicate?: boolean };
  ...
  it('rejects an unparseable time and requires a name', async () => {
    ...
    const bad = await handleAgentTool(ctx, 'book_appointment', { first_name: 'No', last_name: 'Time', appointment_date: 'tomorrow', appointment_time: 'soon' }) as { booked: boolean };
    expect(bad.booked).toBe(false);
    const noName = await handleAgentTool(ctx, 'book_appointment', { appointment_date: futureDate(2), appointment_time: '10:00' }) as { booked: boolean };
  ...
  it('isolates tenants — two clinics can book the same slot independently', async () => {
    ...
    const slot = ((await handleAgentTool({ tenantId: a, callId: 'a' }, 'check_availability', { appointment_date: date })) as { slots: { time: string }[] }).slots[0].time;
    const bookA = await handleAgentTool({ tenantId: a, callId: 'a' }, 'book_appointment', { first_name: 'A', last_name: 'A', appointment_date: date, appointment_time: slot }) as { booked: boolean };
    const bookB = await handleAgentTool({ tenantId: b, callId: 'b' }, 'book_appointment', { first_name: 'B', last_name: 'B', appointment_date: date, appointment_time: slot }) as { booked: boolean };
