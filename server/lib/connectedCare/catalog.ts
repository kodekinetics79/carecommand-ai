...
export const DEVICE_PROVIDERS: DeviceProviderDef[] = [
  { key: 'dexcom', displayName: 'Dexcom', category: 'DIRECT_API', supportsSandbox: true, supportsWebhook: true, configFields: [{ key: 'clientId', label: 'Client ID', secret: false, required: true }, { key: 'clientSecret', label: 'Client secret', secret: true, required: true }], note: 'Continuous glucose. OAuth sandbox available; cleanly NOT_CONFIGURED until credentials exist.' },
  { key: 'withings', displayName: 'Withings', category: 'DIRECT_API', supportsSandbox: true, supportsWebhook: true, configFields: [{ key: 'clientId', label: 'Client ID', secret: false, required: true }, { key: 'clientSecret', label: 'Client secret', secret: true, required: true }], note: 'BP / weight / sleep. OAuth sandbox available; NOT_CONFIGURED until credentials exist.' },
  { key: 'validic', displayName: 'Validic', category: 'AGGREGATOR', supportsSandbox: true, supportsWebhook: true, configFields: [{ key: 'apiKey', label: 'API key', secret: true, required: true }, { key: 'orgId', label: 'Organization ID', secret: false, required: true }], note: 'Multi-device aggregator. NOT_CONFIGURED until credentials exist.' },
  { key: 'terra', displayName: 'Terra', category: 'AGGREGATOR', supportsSandbox: true, supportsWebhook: true, configFields: [{ key: 'apiKey', label: 'API key', secret: true, required: true }, { key: 'devId', label: 'Dev ID', secret: false, required: true }], note: 'Wearable aggregator. NOT_CONFIGURED until credentials exist.' },
  { key: 'tenovi', displayName: 'Tenovi', category: 'RPM_VENDOR', supportsSandbox: true, supportsWebhook: true, configFields: [{ key: 'apiKey', label: 'API key', secret: true, required: true }], note: 'Cellular RPM hub vendor. NOT_CONFIGURED until credentials exist.' },
