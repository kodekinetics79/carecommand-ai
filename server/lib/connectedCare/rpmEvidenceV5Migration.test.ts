...
const migration = readFileSync(
  new URL('../../../prisma/migrations/20260829160000_rpm_evidence_v5_local_period/migration.sql', import.meta.url),
  'utf8',
);

describe('RPM evidence v5 migration', () => {
  ...
  it('grandfathers nothing — both the stale-version and the READY/NEEDS_REVIEW arms are cleared', () => {
    expect(migration).toContain("IS DISTINCT FROM 'rpm-readiness-evidence-v5'");
    expect(migration).toContain("OR \"status\" IN ('READY', 'NEEDS_REVIEW')");
  });

  it('targets the version the code actually emits', () => {
    expect(RPM_EVIDENCE_VERSION).toBe('rpm-readiness-evidence-v5');
