...
export type RemediationTab = 'agent' | 'campaign' | 'intake' | 'clinic' | 'deploy' | 'scheduling' | null;
...
const CATALOGUE = {
  ...
  not_found: { title: 'The agent no longer exists at Retell', action: 'Deploy from Studio to publish a new agent, or link the correct Retell agent id.', scope: 'agent', severity: 'blocking', fixTab: 'deploy', retryable: false },
  invalid_request: { title: 'Retell rejected the request as invalid', action: 'Review the agent voice, language and prompt for values Retell does not accept, then deploy again.', scope: 'agent', severity: 'blocking', fixTab: 'agent', retryable: false },
  provider_invalid_request: { title: 'Retell rejected the deployment as invalid', action: 'Review the agent voice, language and prompt for values Retell does not accept, then deploy again.', scope: 'agent', severity: 'blocking', fixTab: 'agent', retryable: false },
  ...
  deploy_budget_exhausted: { title: 'The deployment ran out of time', action: 'The provider steps that completed were recorded. Deploy again to continue from where it stopped.', scope: 'provider', severity: 'blocking', fixTab: 'deploy', retryable: true },
  ...
  tag_unassigned: { title: 'The deployment tag is not assigned', action: 'Assign the agent’s deployment tag to the published version in the Retell console, then verify again.', scope: 'provider', severity: 'blocking', fixTab: 'deploy', retryable: false },
  ...
  version_mismatch: { title: 'Retell is running a different version', action: 'The published version no longer matches the one CareCommand deployed. Deploy again from Studio.', scope: 'provider', severity: 'blocking', fixTab: 'deploy', retryable: false },
  unpublished: { title: 'The agent version is not published', action: 'Publish the agent version in Retell, or deploy again from Studio, then verify.', scope: 'provider', severity: 'blocking', fixTab: 'deploy', retryable: false },
  webhook_mismatch: { title: 'The agent posts events to the wrong address', action: 'Set the agent webhook to this CareCommand instance, or deploy from Studio, which sets it for you.', scope: 'provider', severity: 'blocking', fixTab: 'deploy', retryable: false },
  webhook_events_mismatch: { title: 'The agent does not send every required call event', action: 'Enable call_started, call_ended and call_analyzed on the agent, or deploy from Studio.', scope: 'provider', severity: 'blocking', fixTab: 'deploy', retryable: false },
  storage_policy_mismatch: { title: 'The agent stores more than CareCommand permits', action: 'Set the agent’s data storage to basic attributes only. Recordings and transcripts must not be retained at the provider.', scope: 'provider', severity: 'blocking', fixTab: 'deploy', retryable: false },
  signed_url_disabled: { title: 'The agent does not use signed URLs', action: 'Enable signed URLs on the agent, or deploy from Studio.', scope: 'provider', severity: 'blocking', fixTab: 'deploy', retryable: false },
  prompt_drift: { title: 'The provider prompt differs from the deployed prompt', action: 'The prompt was edited outside CareCommand. Redeploy from Studio to publish the current prompt, or revert the change in Retell and verify again.', scope: 'provider', severity: 'blocking', fixTab: 'deploy', retryable: false },
  tools_drift: { title: 'The provider tools differ from the deployed tools', action: 'The tool set was edited outside CareCommand. Redeploy from Studio, or revert the change in Retell and verify again.', scope: 'provider', severity: 'blocking', fixTab: 'deploy', retryable: false },
  ...
  provider_deployment_ambiguous: { title: 'More than one agent claims this deployment', action: 'Two agents point at the same Retell agent and version. Deactivate the one that should not answer calls.', scope: 'agent', severity: 'blocking', fixTab: 'agent', retryable: false },
  ...
  provider_response_engine_unsupported: { title: 'This response engine is not supported', action: 'CareCommand attests Retell LLM and Conversation Flow agents. Deploy from Studio to publish a supported engine.', scope: 'provider', severity: 'blocking', fixTab: 'deploy', retryable: false },
  provider_intake_contract_unattested: { title: 'The booking tool could not be attested', action: 'CareCommand could not read exactly one book_appointment tool from the published version. Deploy from Studio.', scope: 'provider', severity: 'blocking', fixTab: 'deploy', retryable: false },
  provider_intake_contract_not_strict: { title: 'Strict tool calling is off', action: 'Enable tool_call_strict_mode on the response engine, or deploy from Studio, which sets it.', scope: 'provider', severity: 'blocking', fixTab: 'deploy', retryable: false },

  // ---- Agent state --------------------------------------------------------
  agent_linked: { title: 'No agent is linked to this campaign', action: 'Create an agent for this clinic and assign it to the campaign.', scope: 'campaign', severity: 'blocking', fixTab: 'agent', retryable: false },
  agent_unlinked: { title: 'The agent is not linked to Retell', action: 'Deploy the campaign to Retell, or link an existing Retell agent id.', scope: 'agent', severity: 'blocking', fixTab: 'deploy', retryable: false },
  agent_unlinked_and_not_creatable: { title: 'This campaign has no agent to deploy', action: 'Create an agent for this clinic and assign it to the campaign, then deploy.', scope: 'campaign', severity: 'blocking', fixTab: 'agent', retryable: false },
  agent_verified: { title: 'The agent is not verified', action: 'Verify the agent against Retell. CareCommand will not place or answer calls on unverified configuration.', scope: 'agent', severity: 'blocking', fixTab: 'deploy', retryable: true },
  agent_unverified: { title: 'The agent is not verified', action: 'Verify the agent against Retell. CareCommand will not place or answer calls on unverified configuration.', scope: 'agent', severity: 'blocking', fixTab: 'deploy', retryable: true },
  agent_inactive: { title: 'The agent is deactivated', action: 'Reactivate the agent, or assign an active agent to this campaign.', scope: 'agent', severity: 'blocking', fixTab: 'agent', retryable: false },
  agent_configuration_changed: { title: 'The agent changed since it was verified', action: 'Verify the agent again so the change is attested before it answers a call.', scope: 'agent', severity: 'blocking', fixTab: 'deploy', retryable: true },
  agent_verification_stale: { title: 'The agent’s verification has expired', action: 'Verify the agent again. Verification is valid for 24 hours and normally renews itself hourly.', scope: 'agent', severity: 'blocking', fixTab: 'deploy', retryable: true },
  agent_scope_mismatch: { title: 'That agent belongs to another clinic', action: 'Choose an agent that belongs to this campaign’s clinic.', scope: 'campaign', severity: 'blocking', fixTab: 'agent', retryable: false },
  engine_not_owned: { title: 'This agent was built outside CareCommand', action: 'Deploying would overwrite an agent CareCommand did not create. Unlink it first, then deploy to publish a fresh agent.', scope: 'agent', severity: 'blocking', fixTab: 'agent', retryable: false },
  concurrent_change: { title: 'The agent changed while this was running', action: 'Somebody edited the agent during the provider call. Nothing was applied. Reload and try again.', scope: 'agent', severity: 'blocking', fixTab: 'deploy', retryable: true },
  ...
  verification_failed: { title: 'The deployment published but did not verify', action: 'The new version is live at Retell but CareCommand could not attest it. Open the agent to see the exact reason and verify again.', scope: 'agent', severity: 'blocking', fixTab: 'deploy', retryable: true },

  // ---- Readiness ----------------------------------------------------------
  deployment_current: { title: 'The deployed prompt is out of date', action: 'The campaign has changed since it was deployed. Deploy the changes so callers hear the current configuration.', scope: 'campaign', severity: 'blocking', fixTab: 'deploy', retryable: false },
  number_bound: { title: 'The phone number does not point at this agent', action: 'Deploy the campaign; deployment binds the Retell number’s inbound agent to the published version.', scope: 'campaign', severity: 'blocking', fixTab: 'deploy', retryable: false },
  ...
  intake_schema_not_strict: { title: 'The booking tool is not strict', action: 'Strict tool calling is required before a booking tool may run. Deploy from Studio, which sets it.', scope: 'campaign', severity: 'blocking', fixTab: 'deploy', retryable: false },
