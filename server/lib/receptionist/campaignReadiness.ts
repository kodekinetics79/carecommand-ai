...
export async function evaluateCampaignReadiness(
  ...
  if (!agent) {
    ...
    if (!deployment) {
      ...
      const changed = deploymentChanges(plan, deployment);
  ...
  const promptConfig = campaignPromptConfig(campaign);
  const placeholders = planDeployment(promptConfig, { mock: retellConfigStatus().mock }).placeholders;
  ...
  const composed = mandatoryOpeningDisclosure(promptConfig);
  checks.push(clinicDisclosure
    ? check('disclosure_composed', 'pass', 'The clinic’s wording is appended to the baseline AI and recording disclosure.', ctx)
    : check('disclosure_composed', 'warn', `Using the product baseline only: “${composed}”`, ctx));
