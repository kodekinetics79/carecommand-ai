...
/** Shape a loaded campaign into the pure prompt config. Shared with the routes. */
export function campaignPromptConfig(campaign: LoadedCampaign): PromptConfig {
  ...
  return {
    ...
    agent: agent
      ? {
        ...
        persona: agent.persona, greetingOverride: agent.greetingOverride,
      }
      ...
      : { name: 'Riley', voice: '11labs-Adrian', tone: 'Warm and professional', language: campaign.clinic.defaultLanguage, persona: null, greetingOverride: null },
    campaign: {
      id: campaign.id,
      name: campaign.name,
      ...
      bookingRules: campaign.bookingRules as PromptConfig['campaign']['bookingRules'],
    ...
    locations: campaign.clinic.locations.map(location => ({
      id: location.id, name: location.name, address: location.address, phone: location.phone,
    })),
    intakeFields: campaign.intakeFields as PromptConfig['intakeFields'],
  };
}
...
export async function deployCampaignToRetell(input: DeployInput): Promise<DeployOutcome> {
  ...
  const claim = await runWithTenantContext(input.tenantId, async tx => {
    ...
    const plan = planDeployment(campaignPromptConfig(campaign), { mock: status.mock });
