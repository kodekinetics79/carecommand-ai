...
import { appendChannelSafetyFooter, buildCampaignDispatchSnapshot, campaignRecipientFirstName } from './campaignIntegrity';
...
export interface DispatchSummary { total: number; accepted: number; deliveryUnknown: number; suppressed: number; skipped: number; setupRequired: number; queued: number; failed: number; authorityBlocked: number; atomicBoundaryBlocked: number }
...
export async function dispatchCampaign(tenantId: string, campaignId: string, opts: { force?: boolean; authorizationFingerprint?: string } = {}): Promise<DispatchSummary & { channel: CommChannel; provider: ReturnType<typeof channelStatus> }> {
  ...
  const s: DispatchSummary = { total: candidates.length, accepted: 0, deliveryUnknown: 0, suppressed: 0, skipped: 0, setupRequired: 0, queued: 0, failed: 0, authorityBlocked: 0, atomicBoundaryBlocked: 0 };
  ...
  for (const cand of candidates) {
    ...
    if (!contact) {
      ...
      // Durable submission claim before the provider boundary. A crash or
      // transport ambiguity leaves delivery_unknown, which automatic retries
      ...
      }); // real provider / dev mock / setup_required / suppressed
      // Provider HTTP acceptance is not delivery. Preserve that distinction
      // until a signed provider receipt advances the state.
      deliveryStatus = result.status === 'sent'
        ? 'accepted'
        : result.status === 'pending'
          ? 'queued'
          : result.failureReason?.startsWith('transport_ambiguous:')
            ? 'delivery_unknown'
            : result.status;
