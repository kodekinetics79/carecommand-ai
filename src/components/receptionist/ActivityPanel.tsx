...
export function ActivityPanel({ clinicId, initialCallId = null, timezone }: {
  clinicId: string;
  /** Deep link from `?callId=`: the named call is opened once the queue settles. */
  ...
  const hydrateReview = useCallback((call: CallLogDetail) => {
  ...
  // `?callId=` opens that call once, after the queue has settled; re-renders
  // and a later manual selection must not reopen it.
  useEffect(() => {
    if (callsState !== 'ready' || !initialCallId || openedInitialCall.current === initialCallId) return;
    openedInitialCall.current = initialCallId;
    void openCall(initialCallId);
  }, [initialCallId, callsState, openCall]);
