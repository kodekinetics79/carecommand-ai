...
export default function CRMMetricsStrip({ m, onNavigate }: { m: CommandMetrics; onNavigate: (route: string) => void }) {
  return (
    <div className="grid gap-3 grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
      ...
      <PremiumMetricCard label="Hot leads" value={m.hotLeads} subtitle="Score ≥ 70" icon={<Flame className="w-4 h-4" />} accent="red" />
      ...
      <PremiumMetricCard label="Inactive recoverable" value={m.inactiveRecoverable} format={formatCurrency} subtitle="Reactivation candidates" icon={<RotateCcw className="w-4 h-4" />} accent="indigo" />
