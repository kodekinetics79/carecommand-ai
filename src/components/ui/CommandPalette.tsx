...
export default function CommandPalette({ isOpen, onClose }: CommandPaletteProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-start justify-center pt-[15vh] cmd-backdrop" onClick={onClose}>
      <div
        className="w-full max-w-xl bg-white rounded-2xl shadow-2xl border border-slate-200/80 overflow-hidden animate-fade-in"
      ...
      >
        ...
        <div className="flex items-center gap-3 px-4 py-3.5 border-b border-slate-100">
          ...
          <div className="flex items-center gap-1.5">
            <kbd className="text-[10px] font-semibold text-slate-400 bg-slate-100 px-1.5 py-0.5 rounded">ESC</kbd>
            <button onClick={onClose} className="w-6 h-6 flex items-center justify-center rounded-lg hover:bg-slate-100 text-slate-400">
          ...
          {Object.entries(grouped).map(([category, items]) => (
            <div key={category}>
              ...
              {items.map((cmd) => (
                ...
                >
                  <div className="w-7 h-7 rounded-lg bg-slate-100 group-hover:bg-blue-100 flex items-center justify-center text-slate-600 group-hover:text-blue-600 transition-colors shrink-0">
                    {cmd.icon}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-slate-900 group-hover:text-blue-700">{cmd.label}</p>
        ...
        <div className="px-4 py-2.5 border-t border-slate-100 flex items-center gap-4 text-[11px] text-slate-400">
          <span className="flex items-center gap-1"><kbd className="bg-slate-100 px-1.5 py-0.5 rounded font-semibold">↑↓</kbd> Navigate</span>
          <span className="flex items-center gap-1"><kbd className="bg-slate-100 px-1.5 py-0.5 rounded font-semibold">↵</kbd> Select</span>
          <span className="flex items-center gap-1"><kbd className="bg-slate-100 px-1.5 py-0.5 rounded font-semibold">⌘K</kbd> Toggle</span>
        </div>
      </div>
    </div>
  );
