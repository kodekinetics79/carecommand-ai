...
| `npm run verify:prisma-drift` | PASS: no unexpected destructive drift; migration-owned composite FKs/indexes recognized |
| `npm run rls:verify` | PASS at current 69-migration checkpoint: 127 application tables, 119 protected, 8 exemptions, 522 policies, ENABLE/FORCE 119/119; safe runtime role |
| `RLS_DISPOSABLE_DB_ACK=CREATE_AND_DROP_LOCAL_RLS_TEST_DATABASE npm run test:rls:behavior` | PASS at current 69-migration checkpoint: 1 file, 962/962 assertions |
...
The scenario catalog contains 45 scenarios, 33 executable scenarios, fixed seed `20260730`, and controlled clock `2026-07-15T14:00:00.000Z`.
