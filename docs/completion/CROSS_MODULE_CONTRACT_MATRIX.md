...
| Receptionist agent deployment | M09 receptionist | M09 outbound and Studio campaigns | published immutable provider version; assigned environment tag; approved webhook/events/storage/signed URLs; same-tenant/clinic fresh readiness checked at activation and dial; exact create-call response or immediate stop/failure; no global fallback | CANDIDATE PASS; independent re-review pending. Live Retell account validation and originating-number/tool-schema gates remain feature-specific |
